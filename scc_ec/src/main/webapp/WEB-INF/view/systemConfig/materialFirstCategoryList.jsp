...
<div>
    <a href="/systemConfig/materialFirstCategory?form">添加产品类别</a>
    <table>
        <tr>
            <td>顺序号</td>
            <td>产品类别</td>
        ...
        <c:forEach var="firstCategory" items="${grid.ecList}" varStatus="list">
            <tr class="tr-${firstCategory.id}">
                <td>${list.index+1}</td>
                <td><a href="/systemConfig/materialSecondCategory/add/${firstCategory.id}">${firstCategory.name}</a></td>
                ...
                <td><a href="#" class="editFirst" data-id="${firstCategory.id}">edit</a></td>
            </tr>
        </c:forEach>
    </table>
</div>